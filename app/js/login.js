	...
	Parse.User.logIn(login, password, {
		success: function (user) {
			if (!user.attributes.business) {
				var errorMessage = "<div class='alert alert-danger' role='alert'>Vous n'avez la permission d'accéder à cette page (compte entreprise requis).</div>";
				$j("#errorAlert").html(errorMessage);	
			} else
				document.location.href = "dashboard.html";
		},
		error: function (user, error) {
			var errorMessage = "<div class='alert alert-danger' role='alert'>E-mail / Mot de passe invalide.</div>";
			$j("#errorAlert").html(errorMessage);
		}
	});
}
